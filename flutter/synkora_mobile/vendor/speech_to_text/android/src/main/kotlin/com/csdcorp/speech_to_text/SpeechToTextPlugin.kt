...
public class SpeechToTextPlugin :
    ...
    private var ignoreRecognizerCallbacks: Boolean = false
    ...
    private fun startListening(result: Result, languageTag: String, partialResults: Boolean,
        ...
        resultSent = false
        ignoreRecognizerCallbacks = false
    ...
    private fun stopListening(result: Result) {
        ...
        ignoreRecognizerCallbacks = true
        handler.post {
            run {
...
//            var recognizerIntent = Intent(RecognizerIntent.ACTION_GET_LANGUAGE_DETAILS)
    ...
    private fun updateResults(speechBundle: Bundle?, isFinal: Boolean) {
        if (ignoreRecognizerCallbacks || isNotListening()) {
            debugLog("Ignoring results after session end")
            return
        }
    ...
    private fun Context.findComponentName(): ComponentName? {
        ...
        val candidates = list.mapNotNull { it.serviceInfo }
        val preferred = candidates.firstOrNull {
            it.packageName == "com.google.android.googlequicksearchbox" &&
                it.name.contains("GoogleRecognitionService")
        } ?: candidates.firstOrNull {
            it.packageName == "com.google.android.tts" &&
                it.name.contains("RecognitionService")
        } ?: candidates.firstOrNull {
            !it.name.contains("AiAiSpeechRecognitionService")
        } ?: candidates.firstOrNull()
        preferred?.let {
            debugLog("RecognitionService selected: ${it.packageName}/${it.name}")
    ...
    override fun onError(errorCode: Int) {
        if (ignoreRecognizerCallbacks || isNotListening()) {
            debugLog("Ignoring error $errorCode after stop/cancel")
            return
        }
        ...
        sendError(errorMsg, isPermanentError(errorReturn))
        if ( isListening()) {
    ...
    private fun sendError(errorMsg: String, permanent: Boolean) {
        val speechError = JSONObject()
        speechError.put("errorMsg", errorMsg)
        speechError.put("permanent", permanent)
        handler.post {
            run {
